...
pub fn shell<T: PartialOrd + Copy>(array: &mut [T]) {
	let mut n = 1;
...
fn insert<T: PartialOrd + Copy>(array: &mut [T], span: usize) {
	// 在 span 跨度下，第一个元素已经算是有序的，所以从 1 * span 开始。
	for mut i in span..array.len() {
		let v = array[i];
...
pub fn binary<T: PartialOrd + Copy>(array: &mut [T]) {
